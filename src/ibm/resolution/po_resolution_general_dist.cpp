...
{
    DistFile << "generation;bin;qS;qNS;s1;s2;prob1;prob2;" << endl;    
}
...
void write_dist()
{
    gsl_histogram * histogram_qS = gsl_histogram_alloc(nbins);
    gsl_histogram * histogram_qNS = gsl_histogram_alloc(nbins);
    gsl_histogram * histogram_s1 = gsl_histogram_alloc(nbins);
    gsl_histogram * histogram_s2 = gsl_histogram_alloc(nbins);
    gsl_histogram * histogram_prob1= gsl_histogram_alloc(nbins);
    gsl_histogram * histogram_prob2= gsl_histogram_alloc(nbins);

    gsl_histogram_set_ranges_uniform(histogram_qS, 0, 1.0001);
    gsl_histogram_set_ranges_uniform(histogram_qNS, 0, 1.0001);
    gsl_histogram_set_ranges_uniform(histogram_s1, 0, 1.0001);
    gsl_histogram_set_ranges_uniform(histogram_s2, 0, 1.0001);
    gsl_histogram_set_ranges_uniform(histogram_prob1, 0, 1.0001);
    gsl_histogram_set_ranges_uniform(histogram_prob2, 0, 1.0001);

    for (int i = 0; i < Npatches; ++i)
    {
        for (int j = 0; j < Npp; ++j)
        {
            gsl_histogram_increment(histogram_qS,MetaPop[i].locals[j].qS[0]+ MetaPop[i].locals[j].qS[1]);
            gsl_histogram_increment(histogram_qNS,MetaPop[i].locals[j].qNS[1] + MetaPop[i].locals[j].qNS[1]);
            gsl_histogram_increment(histogram_s1,MetaPop[i].locals[j].s[0][0] + MetaPop[i].locals[j].s[1][0]);
            gsl_histogram_increment(histogram_s2,MetaPop[i].locals[j].s[0][1] + MetaPop[i].locals[j].s[1][1]);
            gsl_histogram_increment(histogram_prob1,
                    (MetaPop[i].locals[j].s[0][0] + MetaPop[i].locals[j].s[1][0]) * (MetaPop[i].locals[j].qS[0] + MetaPop[i].locals[j].qS[1])
                    +
                    (1-(MetaPop[i].locals[j].s[0][0] + MetaPop[i].locals[j].s[1][0])) * (MetaPop[i].locals[j].qNS[0] + MetaPop[i].locals[j].qNS[1])
                    );
            gsl_histogram_increment(histogram_prob2,
                    (MetaPop[i].locals[j].s[0][1] + MetaPop[i].locals[j].s[1][1]) * (MetaPop[i].locals[j].qS[0] + MetaPop[i].locals[j].qS[1])
                    +
                    (1-(MetaPop[i].locals[j].s[0][1] + MetaPop[i].locals[j].s[1][1])) * (MetaPop[i].locals[j].qNS[0] + MetaPop[i].locals[j].qNS[1])
                    );
        }
    }

    for (size_t i = 0; i < gsl_histogram_bins(histogram_qS); ++i)
    {
        DistFile << generation << ";" << (double) i / gsl_histogram_bins(histogram_qS) << ";" << gsl_histogram_get(histogram_qS, i) << ";"
            << gsl_histogram_get(histogram_qNS, i) << ";"
            << gsl_histogram_get(histogram_s1, i) << ";"
            << gsl_histogram_get(histogram_s2, i) << ";" 
            << gsl_histogram_get(histogram_prob1, i) << ";"
            << gsl_histogram_get(histogram_prob2, i) << ";" 
            << endl;
    }

    DistFile << generation << ";1.0;0;0;0;0;" << endl;
    
    gsl_histogram_free(histogram_qS);
    gsl_histogram_free(histogram_qNS);
    gsl_histogram_free(histogram_s1);
    gsl_histogram_free(histogram_s2);
    gsl_histogram_free(histogram_prob1);
    gsl_histogram_free(histogram_prob2);
}
